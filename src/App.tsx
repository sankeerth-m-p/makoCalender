    ...
    <div className="min-h-screen bg-[#f6f7fb] flex items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl border bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-base font-bold">Makoni's Trade Intelligence</div>
...
function SheetTab({ active, children, onClick }: SheetTabProps): JSX.Element {
  return (
    <button
      onClick={onClick}
      className={classNames(
        "w-full rounded-xl px-4 py-2 text-sm font-semibold sm:w-auto",
        active ? "bg-[#0b5cad] text-white" : "bg-slate-100"
      )}
    >
      {children}
    </button>
